...
<!--                                <div class="row">-->
<!--                                    <div class="col-sm-8">-->
<!--                                        <span class="text">{$user_detail[0]->employee_no} </span>-->
<!--                                    </div>-->
<!--                                </div>-->
<!--                            </div>-->
<!--                        </div>-->
                        ...
                        <div class="form-footer">
                            <div class="pull-right">
                                {if in_array('user/delete_user',$pages) }
                                {if $user_detail[0]->id != $user_id  }
                                {if $user_detail[0]->level < $user_level || $user_id == 1}
                                <button class="btn btn-danger btn-stroke" id="complexConfirm"><i class="fa fa-trash" aria-hidden="true"></i> Delete</button>
                                {/if}
                                {/if}
                                {/if}
                                {if in_array('user/edit',$pages) }

                                <a class="btn btn-twitter btn-stroke"  href="{base_url('user/edit')}/{$user_detail[0]->id}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a>
                                {/if}

                                {if in_array('user/change_login',$pages) && {$user_id==1} }

                                <a class="btn btn-success btn-stroke"  href="{base_url('user/change_login')}/{$user_detail[0]->id}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Login</a>
                                {/if}
                            </div>
                            <div class="clearfix"></div>
                        </div>
